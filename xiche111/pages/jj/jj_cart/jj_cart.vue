<template>
	<view>
		<!-- #ifdef H5 -->
		<pc-sidebar v-if="isPC" active="cart" :menu-list="pcMenuList" />
		<!-- #endif -->
		...
		<jj-tabbar v-if="isMobile" active="cart" />
	</view>
</template>
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				pcMenuList: [
					...
					{ key: 'cart', label: '商品清单', icon: '/static/icon/icon_cart_uc.svg', iconActive: '/static/icon/icon_cart_sc.svg', url: '/pages/jj/jj_cart/jj_cart', isTab: false },
		...
		methods: {
			goProducts() {
				uni.redirectTo({ url: '/pages/jj/jj_main/jj_main?tab=products' });
			},
			loadCart() {
				this.$core.get({
					...
					success: ret => {
						...
						list.forEach(item => {
							...
							let depositRate = Number(item.deposit_rate) || 10;
