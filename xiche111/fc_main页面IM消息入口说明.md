...
工厂端首页 `pages/fc/fc_main/fc_main` 的快捷入口中没有消息入口，需要补充IM消息功能。
...
`fc_main` 是一个容器页面，采用组件化设计：

```
pages/fc/fc_main/fc_main.vue (容器页面)
  ↓
components/fc-home-content/fc-home-content.vue (首页内容组件)
  ↓ 显示
快捷入口区域 (包含消息入口)
```

实际的首页内容在 `fc-home-content` 组件中，所以需要修改这个组件文件。

---

## 🔧 已完成的修改

### 文件: `/components/fc-home-content/fc-home-content.vue`

#### 1. 快捷入口区域（第97-138行）

**修改前**（4个图标）:
```
[产品管理] [订单管理] [竞标管理] [居间人筛选]
```

**修改后**（5个图标，分2行显示）:
```
第1行: [产品管理] [消息💬] [订单管理] [竞标管理]
第2行: [居间人筛选]
```
...
```
http://192.168.3.19:8082/web/#/pages/fc/fc_main/fc_main
```
...
用户访问 http://192.168.3.19:8082/web/
  ↓
进入 pages/fc/fc_main/fc_main
  ↓
加载 fc-home-content 组件
  ↓ mounted()
调用 loadUnreadCount()
...
`fc_main` 是容器页面，包含多个Tab：
- **home**: 首页（fc-home-content组件）
- **products**: 产品（fc-products-content组件）
- **orders**: 订单（fc-orders-content组件）
- **profile**: 我的（fc-profile-content组件）
...
- ✅ `/components/fc-home-content/fc-home-content.vue` - 已修改
- ✅ `/pages/fc/fc_main/fc_main.vue` - 容器页面（无需修改）
...
**测试方法**:
1. 清除浏览器缓存
2. 访问 `http://192.168.3.19:8082/web/#/pages/fc/fc_main/fc_main`
3. 查看快捷入口第2个图标（消息 💬）
4. 点击测试功能
